...
!MESSAGE 
!MESSAGE NMAKE /f "pennmush.mak" CFG="pennmush - Win32 SSL Debug"
!MESSAGE 
...
# ADD LINK32 user32.lib winmm.lib wsock32.lib /nologo /subsystem:console /machine:I386
# Begin Custom Build - Copying Necessary Files
ProjDir=.
InputPath=.\game\pennmush.exe
SOURCE="$(InputPath)"

BuildCmds= \
	IF NOT EXIST $(ProjDir)\src\cmdlocal.c copy $(ProjDir)\src\cmdlocal.dst $(ProjDir)\src\cmdlocal.c \
	IF NOT EXIST $(ProjDir)\src\flaglocal.c copy $(ProjDir)\src\flaglocal.dst $(ProjDir)\src\flaglocal.c \
	IF NOT EXIST $(ProjDir)\src\funlocal.c copy $(ProjDir)\src\funlocal.dst $(ProjDir)\src\funlocal.c \
	IF NOT EXIST $(ProjDir)\src\local.c copy $(ProjDir)\src\local.dst $(ProjDir)\src\local.c \
	IF NOT EXIST $(ProjDir)\game\mush.cnf echo Don't forget to customize mush.cnf! \
	IF NOT EXIST $(ProjDir)\game\mush.cnf copy $(ProjDir)\game\mushcnf.dst $(ProjDir)\game\mush.cnf \
	copy $(ProjDir)\win32\cmds.h $(ProjDir)\hdrs\cmds.h \
	copy $(ProjDir)\win32\funs.h $(ProjDir)\hdrs\funs.h \
	echo If any *local.c files failed to build, run build again. \
	

"$(ProjDir)\src\cmdlocal.c" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\src\flaglocal.c" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\src\funlocal.c" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\src\local.c" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\game\mush.cnf" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\hdrs\cmds.h" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)

"$(ProjDir)\hdrs\funs.h" : $(SOURCE) "$(INTDIR)" "$(OUTDIR)"
   $(BuildCmds)
# End Custom Build
...
# ADD LINK32 kernel32.lib gdi32.lib winspool.lib comdlg32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib user32.lib winmm.lib wsock32.lib /nologo /subsystem:console /debug /machine:I386 /pdbtype:sept
# Begin Custom Build - Copying Necessary Files
ProjDir=.
InputPath=.\game\pennmush.exe
SOURCE="$(InputPath)"
...
# ADD LINK32 user32.lib winmm.lib wsock32.lib ssleay32.lib libeay32.lib /nologo /subsystem:console /pdb:"pennmush.pdb" /machine:I386
...
# ADD LINK32 kernel32.lib gdi32.lib winspool.lib comdlg32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib user32.lib winmm.lib wsock32.lib ssleay32.lib libeay32.lib /nologo /subsystem:console /debug /machine:I386 /pdbtype:sept
# Begin Custom Build - Copying Necessary Files
ProjDir=.
InputPath=.\game\pennmush.exe
SOURCE="$(InputPath)"
